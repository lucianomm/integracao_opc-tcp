	...
	HRESULT *pErrors)
{
	...
	for (DWORD dwItem = 0; dwItem < dwCount; dwItem++)
	{
		...
		{
		case 0:
			this->sLeitura[0] = pvValues[dwItem];
			this->sHandleLeitura[0] = phClientItems[dwItem];
			break;
		case 1:
			this->sLeitura[1] = pvValues[dwItem];
			this->sHandleLeitura[1] = phClientItems[dwItem];
			break;
		case 2:
			this->sLeitura[2] = pvValues[dwItem];
			this->sHandleLeitura[2] = phClientItems[dwItem];
			break;
		case 3:
			this->sLeitura[3] = pvValues[dwItem];
			this->sHandleLeitura[3] = phClientItems[dwItem];
			break;
		}
...
VARIANT* SOCDataCallback::sendValues() {
	return this->sLeitura;
}

OPCHANDLE* SOCDataCallback::sendHandles() {
	return this->sHandleLeitura;
}